...
Direct port from day11.el maintaining same data structures and algorithms.
"""
import sys
sys.path.insert(0, '/Users/dpzmick/programming/elisp-aoc/python')

from aoc_common import RowMajorMat, read_input, read_sample, expect, check_result
...
def intern_name(name: str, table: dict[str, int]):
...
def fill_mat_line(line: str, adj: RowMajorMat, table: dict[str, int]):
...
def process_input(input_str: str) -> tuple[dict[str, int], RowMajorMat]:
    """Parse input into node table and adjacency matrix."""
    table = make_node_table(input_str)
    n = len(table)
    adj = RowMajorMat(n, n, initial_value=0)
...
def dfs(from_node: int, to_node: int, adj: RowMajorMat) -> int:
    ...
    total = 0
    for i in range(adj.cols):
        if adj.ref(from_node, i) == 1:
...
def dfs_p2(from_node: int, to_node: int, dac_id: int, fft_id: int, adj: RowMajorMat) -> int:
    """Count paths that visit both dac and fft nodes."""
    memo = {}

    def dfs_inner(from_node: int, to_node: int, seen_dac: bool, seen_fft: bool) -> int:
        # Update seen flags

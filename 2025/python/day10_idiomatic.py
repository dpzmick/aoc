...
# Matrix operations for Gaussian elimination with NumPy
def row_gcd_np(row: np.ndarray) -> int:
    """Get GCD of all non-zero values in a row."""
    non_zero = row[row != 0]
    if len(non_zero) == 0:
        return 1
    return reduce(gcd, map(abs, non_zero))


def normalize_row_np(row: np.ndarray) -> np.ndarray:
...
def swap_rows_np(m: np.ndarray, r1: int, r2: int):
    """Swap two rows."""
    if r1 != r2:
        m[[r1, r2]] = m[[r2, r1]]


def add_rows_np(m: np.ndarray, r1: int, r2: int, factor1: int, factor2: int):
    """Add scaled rows: r2 = factor1 * r1 + factor2 * r2."""
    m[r2] = factor1 * m[r1] + factor2 * m[r2]


def nonzero_row_idx_for_col(m: np.ndarray, c: int, st: int = 0):
    """Find first row (>= st) with nonzero value in column c."""
    for r in range(st, m.shape[0]):
        if m[r, c] != 0:
            return r
    return None


def eliminate_down(m: np.ndarray, pivot_r: int, c: int):
    """Eliminate all values below pivot."""
    for r in range(pivot_r + 1, m.shape[0]):
        factor1 = m[r, c]  # scale by our val
        factor2 = -m[pivot_r, c]  # scale by pivot (negated)
        add_rows_np(m, pivot_r, r, factor1, factor2)
        m[r] = normalize_row_np(m[r])


def eliminate_up(m: np.ndarray, pivot_r: int, c: int):
    ...
    for r in range(pivot_r):
        ...
        if target_val != 0:
            ...
            add_rows_np(m, pivot_r, r, -target_val, pivot_val)
            m[r] = normalize_row_np(m[r])


def eliminate(m: np.ndarray) -> list[tuple[int, int]]:
    ...
    for c in range(m.shape[1]):
        ...
        if tgt_r is not None:
            swap_rows_np(m, pivot_row, tgt_r)
...
def rhs_col(m: np.ndarray) -> int:
    """Column index of RHS."""
    return m.shape[1] - 1


def num_vars(m: np.ndarray) -> int:
...
def free_var_cols(m: np.ndarray, pivot_cols: list[tuple[int, int]]) -> list[int]:
    """Find columns that are free variables."""
    pcols = pivot_col_set(pivot_cols)
    return [c for c in range(num_vars(m)) if c not in pcols]


def pivot_row_effective_rhs(m: np.ndarray, pivot_row: int, free_vals: list[tuple[int, int]]) -> int:
    """Compute effective RHS after plugging in free variables."""
    adjustment = sum(m[pivot_row, fc] * fv for fc, fv in free_vals)
    return m[pivot_row, rhs_col(m)] - adjustment


def solve_pivot_var(m: np.ndarray, pivot_row: int, pivot_col: int, free_vals: list[tuple[int, int]]) -> int:
...
def solve_system(m: np.ndarray, pivot_cols: list[tuple[int, int]], free_vals: list[tuple[int, int]]) -> list[tuple[int, int]]:
...
def integer_solution_p(m: np.ndarray, pivot_cols: list[tuple[int, int]], free_vals: list[tuple[int, int]]) -> bool:

...
// return the amount of ore required to convert the entire inventory to ore
fn solve(reactions: &[Reaction], names: &NameID) -> usize {
    ...
    loop {
        ...
        println!("target: {} ({})", names.get_by_id(chem).unwrap(), chem);
        ...
        let n = (amount as f64/ reaction.input.1 as f64).ceil() as i64;

        for i in 0..n {
            if reaction.input.1 <= inventory[reaction.input.0] {
                inventory[reaction.input.0] -= reaction.input.1;
            }
            else {
...
fn main() {
    ...
    println!("ans: {}", solve(&reactions, &names));
}

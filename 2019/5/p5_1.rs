...
fn main() {
    ...
    loop {
        ...
        match op {
            ...
            5 => flags = numbers[pidx(0)] > 0,
            6 => flags = numbers[pidx(0)] == 0,
            7 => { let idx = pidx(2); numbers[idx] = if numbers[pidx(0)] < numbers[pidx(1)] { 1 } else { 0 } },
            8 => { let idx = pidx(2); numbers[idx] = if numbers[pidx(0)] == numbers[pidx(1)] { 1 } else { 0 } },
            _ => panic!("bad op"),
        };

        // FIXME hack, can't use the pidx() function again in the pc
        // adjustment because:
        // 1) closure is holding an immutable ref
        // 2) a mutalbe ref is taken above to assign to numbers
        // if we don't do something to drop then restablish our
        // immatable ref, the captured immutable ref crosses the mutable ref.
        let pidx = |idx| {
            let spec = (if idx == 0 { param_spec } else { (param_spec/(10*idx)) })%10;
            match spec {
                0 => numbers[pc+idx+1] as usize,
        ...
        pc = match op {
            ...
            3 => pc + 2,
            4 => pc + 2,
            5 => if flags { numbers[pidx(1)] as usize } else { pc + 3 },
            6 => if flags { numbers[pidx(1)] as usize } else { pc + 3 },
            7 => pc + 4,
            8 => pc + 4,
            _ => panic!("bad op"),
        }
